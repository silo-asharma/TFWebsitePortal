app.controller('ExporterCtrl', function ($scope, $http) {
    ...
    $scope.getExporterGraph = function () {
        ...
        $http.get("http://localhost:54863/Search/MGetDashboardlist?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSizeSelected + "&mode=" + $scope.mode + "&year=" + $scope.year + "&from=" + $scope.from + "&to=" + $scope.to + "&header=" + $scope.header + "&keyword=" + $scope.keyword).then(
